                ...
                <ProductConsumer>
                    {(value) => (
                        ...
                        <button className="cart-btn" disabled={inCart ? true : false} onClick={() => { value.addToCart(id); value.openModal(id); }} >
                            ...
                            {inCart ? (<p className="text-capitalized mb-0" disabled>{" "}In Cart</p>) : (<i className="fas fa-cart-plus" />) }
...
Product.propTypes = {//Stating the kind of data expected in the props/"attributes" of each product object
...
.cart-btn {
    ...
    transform: translate(100%, 100%);//hide the button out of sight until hovering
    transition: all 0.2s linear;
}
